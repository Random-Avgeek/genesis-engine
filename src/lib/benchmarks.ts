...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Racer Benchmark</title>
    <script src="https://unpkg.com/kaplay@3001.0.0-alpha.21/dist/kaplay.js"></script>
    <style>
        body { margin: 0; display: flex; height: 100vh; background: #050505; color: white; font-family: monospace; overflow: hidden; }
        #viewport { flex-grow: 1; position: relative; background: #050510; }
        #hud { position: absolute; top: 20px; left: 20px; z-index: 10; background: rgba(0,0,0,0.8); padding: 15px 20px; border: 1px solid #333; border-left: 3px solid #ff0055; border-radius: 4px; pointer-events: none; }
        .hud-line { font-size: 1.2rem; font-weight: bold; color: #aaa; margin-bottom: 5px; }
        .hud-line span { color: #ff0055; }
        .instruction { position: absolute; bottom: 20px; left: 0; width: 100%; text-align: center; color: #888; font-size: 1.2rem; z-index: 10; pointer-events: none; }
        #equation { font-size: 3rem; color: #fff; text-shadow: 0 0 15px #ff0055; }
    </style>
</head>
<body>
    <div id="viewport">
        <div id="hud">
            <div class="hud-line">SECTORS CLEARED: <span id="score">0</span></div>
            <div id="equation">? x ?</div>
        </div>
        <div class="instruction">USE [LEFT] AND [RIGHT] ARROWS TO HIT THE CORRECT GATE</div>
    </div>
    ...
    <script>
        ...
        scene("main", () => {
            ...
            loop(0.1, () => {
                add([
                    rect(2, rand(10, 30)), pos(rand(0, width()), -50), color(200, 200, 255),
                    opacity(rand(0.2, 0.6)), move(DOWN, speed * 2), lifespan(3), z(0)
                ]);
            });
            ...
            const ship = add([
                text("🚀", { size: 60 }),
                pos(lanes[currentLane], height() - 100),
                anchor("center"),
                area({ scale: 0.5 }),
                z(10),
                "player"
            ...
            // Movement
            onKeyPress("left", () => {
                if (currentLane > 0) { currentLane--; tween(ship.pos.x, lanes[currentLane], 0.1, (v) => ship.pos.x = v, easings.easeOutQuad); }
            });
            onKeyPress("right", () => {
                if (currentLane < 2) { currentLane++; tween(ship.pos.x, lanes[currentLane], 0.1, (v) => ship.pos.x = v, easings.easeOutQuad); }
            });

            // Math Logic
            function spawnGates() {
                const num1 = randi(2, 12);
                const num2 = randi(2, 12);
                const ans = num1 * num2;
                document.getElementById('equation').innerText = \`\${num1} x \${num2}\`;

                const correctLane = randi(0, 3);
                
                for(let i=0; i<3; i++) {
                    let val = i === correctLane ? ans : ans + randi(-5, 5) + (i*2+1);
                    if (i !== correctLane && val === ans) val += 1; // Ensure fake isn't real

                    const gate = add([
                        rect(laneWidth - 20, 20),
                        pos(lanes[i], -50),
                        anchor("center"),
                        color(i === correctLane ? 0 : 255, 0, 0), // Hidden visually, relies on math
                        opacity(0.1),
                        area(),
                        move(DOWN, speed),
                        "gate",
                    ...
                    gate.add([ text(val.toString(), { size: 40 }), color(255, 255, 255), anchor("center") ]);
                }
            }

            wait(1, spawnGates);

            onCollide("player", "gate", (p, g) => {
                if (g.isCorrect) {
                    score++;
                    speed += 20; // Increase speed
                    document.getElementById('score').innerText = score;
                    destroyAll("gate");
                    add([ text("⚡", {size: 60}), pos(p.pos), anchor("center"), lifespan(0.2) ]);
                    wait(0.5, spawnGates);
                } else {
                    shake(20);
                    score = 0;
            ...
            // Cleanup missed gates
            onUpdate("gate", (g) => { if (g.pos.y > height() + 100) destroy(g); });
        });
